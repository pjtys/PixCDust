...
def gdf_to_h3_gdf(
    ...
    gdf[h3_col] = gdf.apply(
        lambda row: h3.api.basic_int.geo_to_h3(
...
def gdf_to_healpix_gdf(
    ...
    gdf[healpix_col] = gdf.apply(
        lambda row: healpix.lonlat_to_healpix(
            row["longitude"] * u.deg, row["latitude"] * u.deg
        ),
        axis=1,
    )
    ...
    geometry = healpix_df[healpix_col].apply(
        lambda pix: healpix_to_polygon(healpix, pix)
    )
